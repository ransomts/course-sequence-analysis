...
knitr::opts_chunk$set(tidy = TRUE, cache = FALSE, message = FALSE, error = TRUE)
...
most_represented_majors <- term %>% 
  ...
  select(cip6) %>% 
  arrange(cip6) %>% 
  summarise(count = n(), .by = cip6) %>% 
  arrange(-count) 

most_represented_majors %>% 
...
total_counts <- when_courses_were_taken %>% 
  summarise(total_count = sum(COUNT), .by = course)
...
when_courses_were_taken_discrete %>% 
  filter(course %in% highly_taken_classes$course) %>%
...
make_graph <- function(single_student_sequence) {
  return(tidygraph::tbl_graph(
...
make_course_graph <- function(single_student_record) {
  ...
  return(tbl_graph(
    nodes = make_course_nodes(single_student_record$data[[1]]),
    edges = make_course_edges(single_student_record$data[[1]])
    ))
}
...
graph_c <- graph_join(graph_a, graph_b, 
                      by = join_by(course, term_course, node_id))
vis_graph <- toVisNetworkData(graph_c)
...
visNetwork(vis_graph$nodes, vis_graph$edges, height = "500px", width = "100%") %>% 
  visInteraction(navigationButtons = TRUE) %>%
  visEdges(arrows = 'to') %>% 
  visExport()
